...
export const relationshipService = {
  ...
  ): Promise<RelationshipResponse<RelationshipItem>> => {
    try {
  ...
  getRelationships: async (): Promise<
    RelationshipResponse<RelationshipItem[]>
  > => {
    try {
      const response =
        await apiService.get<RelationshipResponse<RelationshipItem[]>>(
          "/relationships",
        );
      return response;
    } catch (error) {
  ...
  deleteRelationship: async (codigo: string): Promise<RelationshipResponse> => {
    try {
      const response = await apiService.delete<RelationshipResponse>(
        `/relationships/${codigo}`,
      );
      return response;
    } catch (error) {
