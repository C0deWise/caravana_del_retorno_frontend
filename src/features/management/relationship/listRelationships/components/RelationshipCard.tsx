...
export function RelationshipCard({ relationship, index, targetUserId }: RelationshipCardProps) {
    
    const { relationshipType, status, user, relatedUser } = relationship;
    const displayUser = user.codigo === targetUserId ? relatedUser : user;
    const displayUserName = `${displayUser.nombre} ${displayUser.apellido}`.trim();
    ...
    return(
        <div className="bg-bg border border-gray-100 rounded-4x1 p-6 shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold border ${statusStyles}`}>
                    {statusLabel}
                </span>
            </div>
        </div>
