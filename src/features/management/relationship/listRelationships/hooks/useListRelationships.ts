 ...
 */
const normalizeStatus = (status?: string): 'pendiente' | 'aceptada' | 'rechazada' | null => {
    if (!status) {
        return null;
    }
    ...
    if (normalized === 'pendiente' || normalized === 'aceptada' || normalized === 'rechazada') {
        return normalized;
    }
 ...
 */
export function useListRelationships(targetUserId: string): UseListRelationshipsReturn {
    ...
    const filteredRelationships = useMemo(
        () =>
            allRelationships.filter(
                (relationship) =>
                    relationship.user.codigo === targetUserId || relationship.relatedUser.codigo === targetUserId,
            ),
    ...
    const currentPage =
        pagination.targetUserId === targetUserId
            ? pagination.page
            : 1;

    const relationships = useMemo(() => {
        const paginated = prioritizedRelationships.slice(0, currentPage * PAGE_SIZE);
